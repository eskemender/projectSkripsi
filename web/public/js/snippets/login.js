        ...
        init:function(){
            ...
            $("#login_form").submit(function(e){
                ...
                var a = $('#m_login_signin_submit'),
                    ...
                    var array_data  = {};
                    var c   = ajax.controller('login');        
                    var m   = ajax.method('check');        
                    var ajax_data   = ajax.ajax_data(c,m);    
                    
                    array_data['element']  = this;
                    array_data['data']     = ajax_data;        
                    ajax.request(array_data,'login');
            });
        }
    }
}();
...
function login(response) {
    if(response === "empty")
    {
        show_toastr('Masih ada field yang kosong!', 'warning', 'Opps!');
    }
    else if(response === "fail")
    {
        show_toastr('Login failed!', 'error', 'Opps!');
    }
    ...
    {
        show_toastr(response, 'error', 'Opps!');
    }
}